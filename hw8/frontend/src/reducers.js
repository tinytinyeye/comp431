...
function profile(state = { name: '', email: '', zipcode: '',
	avatar : '', headline: '', birthday: ''}, action) {
	switch (action.type) {
		case "UPDATE_AVATAR":
...
function articles(state = { articles: {}, avatars: {}, keyword: "" }, action) {
	...
	switch (action.type) {
		...
		case "UPDATE_ARTICLE":
			...
			return { ...state, articles: {...articles, article} }
...
function loginType(state = { loginAs: 'local' }, action) {
	switch (action.type) {
		case "LOCAL":
			return { loginAs: 'local' }
		case "GOOGLE":
			return { loginAs: 'google' }
		case "LINKED":
			return { loginAs: 'linked' }
		default:
			return state;
	}
}
...
const Reducer = combineReducers( { profile, route,
	status, following, articles, loginType } )
