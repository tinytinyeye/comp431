...
const resource = (method, endpoint, payload) => {
	const url = `https://comp431-backend.herokuapp.com/${endpoint}`
...
describe('Test profile stubs', () => {
	...
	it('should PUT headline', done => {
		...
		.then(resource('GET', 'headlines').then(body => {
				expect(body.headlines[0].username).to.be.ok
				expect(body.headlines[0].headline).to.be.eql(now)
		}))
		.then(done).catch(done)
	})
